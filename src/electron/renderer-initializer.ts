...
import { ElectronLink } from './views/device-connect-view/components/electron-link';
import { sendAppInitializedTelemetryEvent } from './views/device-connect-view/send-app-initialized-telemetry';
import { RootContainerRenderer } from './views/root-container/root-container-renderer';
...
getPersistedData(indexedDBInstance, indexedDBDataKeysToFetch).then((persistedData: Partial<PersistedData>) => {
    ...
    const windowStateStore = new WindowStateStore(new WindowStateActions());
